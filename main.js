let products=[
    ...
    let products_container= document.getElementById("container")
    let all_products= document.getElementById("all-products")
    let mens_products= document.getElementById("mens")
    let womens_products= document.getElementById("womens")
    let electronics_products= document.getElementById("electronics")
    let jewelery_products= document.getElementById("jewelery")

    

    function productCategories(c){
        c.filter((val,ind,arr)=>{
            ...
            // products_container.appendChild(product_image_container)

            // let product_title= document.createElement("p")
            // products_container.appendChild(product_title)
            ...
            // product_description.innerText=val.description
            // product_card.appendChild(product_description)

            let product_price= document.createElement("p")
            product_price.setAttribute("class","product-price")
            product_price.innerHTML=`RS. <span class="price-color">${val.price}</span>`
            ...
            wishlist_icon.setAttribute("class","card-wishlist-icon")
            product_card.appendChild(wishlist_icon)

            let wishlist_iconadding= document.createElement("i")
            wishlist_iconadding.setAttribute("class","fa-solid fa-heart fav-icon")
            wishlist_iconadding.addEventListener("click",()=>{

              
                let wishlist=JSON.parse(localStorage.getItem("wishlist")) || []
                // let wishlistt=wishlist!=null?JSON.parse(wishlisst):[];
                wishlist.push(c[ind])
                localStorage.setItem("wishlist",JSON.stringify(wishlist))
                alert("atem idded to wishlist")

            })
            wishlist_icon.appendChild(wishlist_iconadding)
            ...
            addToCart.addEventListener("click",()=>{
                ...
                //      return  a.length
                // }
                // console.log(cartcountt(cart))
                // let cartcount=cartcountt(cart)
                // cartCount.innerText=`${cartcount}`
            })
            product_card.appendChild(addToCart)
    ...
    mens_products.addEventListener("click",()=>{
        var mens=[]
        for(i=0;i<products.length;i++){
            if(products[i].category=="men's clothing"){
                mens.push(products[i])
            }
    ...
    womens_products.addEventListener("click",()=>{
        var womens=[]
        for(i=0;i<products.length;i++){
            if(products[i].category=="women's clothing"){
                womens.push(products[i])
            }
        }
        clear()
        productCategories(womens)
    })
    electronics_products.addEventListener("click",()=>{
        var elec=[]
        for(i=0;i<products.length;i++){
        ...
        clear()
        productCategories(elec)
    })
    jewelery_products.addEventListener("click",()=>{
        var jewel=[]
        for(i=0;i<products.length;i++){
            if(products[i].category=="jewelery"){
                jewel.push(products[i])
            }
        }
        clear()
        productCategories(jewel)
    })

    let profilegetting=document.getElementById("login-signup")
    profilegetting.addEventListener("click",()=>{
